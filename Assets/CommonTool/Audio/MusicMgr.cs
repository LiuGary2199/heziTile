...
public class MusicMgr : MonoSingleton<MusicMgr>
{
    ...
    //背景音乐开关
    private bool _BgMusicSwitch=true;
    //音效开关
    private bool _EffectMusicSwitch = true;
    //音乐音量
    ...
    {
        if (!SaveDataManager.GetBool("first_music_set"))
        {
    ...
    private void PlayBgBase(object bgName, bool restart = false)
    {
        ...
        AudioClip clip = MusicResourcesMgr.GetInstance().Load<AudioClip>(bgName);
        //如果找到了，不为空
        if (clip != null)
        {
    ...
    private void PlayEffectBase(object effectName, bool defAudio = true, float volume = 1f)
    {
        ...
        m_effectMusic.volume = _EffectVolume;
        //根据查找路径加载对应的音频剪辑
        AudioClip clip =MusicResourcesMgr.GetInstance().Load<AudioClip>(effectName);
        //如果为空的画，直接报错，然后跳出
        if (clip == null)
        {
