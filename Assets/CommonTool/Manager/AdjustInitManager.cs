...
public class AdjustInitManager : MonoBehaviour
{
    ...
    public void InitAdjustData(bool isOldUser = false)
    {
        ...
        if (string.IsNullOrEmpty(NetInfoMgr.instance.NetServerData.adjust_init_act_position) || int.Parse(NetInfoMgr.instance.NetServerData.adjust_init_act_position) <= 0)
        {
            SaveDataManager.SetString(sv_ADJustInitType, AdjustStatus.OpenAsAct.ToString());
        }
    ...
    public void AddActCount(string param2 = "")
    {
        ...
        if (string.IsNullOrEmpty(NetInfoMgr.instance.NetServerData.adjust_init_act_position) || _currentCount == int.Parse(NetInfoMgr.instance.NetServerData.adjust_init_act_position))
        {
    ...
    public void AddAdCount(string countryCode, double revenue)
    {
        ...
        if (!string.IsNullOrEmpty(NetInfoMgr.instance.NetServerData.adjust_init_adrevenue))
        {
            JsonData jd = JsonMapper.ToObject(NetInfoMgr.instance.NetServerData.adjust_init_adrevenue);
        ...
        if (
            string.IsNullOrEmpty(NetInfoMgr.instance.NetServerData.adjust_init_act_position)                   //后台没有配置限制条件，直接走LoadAdjust
            || (_currentCount == int.Parse(NetInfoMgr.instance.NetServerData.adjust_init_act_position)         //累计广告次数满足adjust_init_act_position条件，且累计广告收入满足adjust_init_adrevenue条件，走LoadAdjust
    ...
    public void LoadAdjustOnAct(string param2 = "")
    {
        ...
        if (string.IsNullOrEmpty(NetInfoMgr.instance.NetServerData.adjust_init_rate_act) || int.Parse(NetInfoMgr.instance.NetServerData.adjust_init_rate_act) > Random.Range(0, 100))
        {
