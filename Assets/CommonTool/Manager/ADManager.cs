...
public class ADManager : MonoBehaviour
{
    ...
    bool autoInter = false;
    float autoInterTime = 0;
    ...
    private void playInterstitial(int index, int customIndex = 0)
    {
        ...
        int sv_trialNum = SaveDataManager.GetInt(CConfig.sv_Level_Id);
        int trial_MaxNum = int.Parse(NetInfoMgr.instance.NetServerData.trial_MaxNum);
        ...
        // 时间间隔低于阈值，不播放广告
        if (lastPlayTimeCounter < int.Parse(NetInfoMgr.instance.NetServerData.inter_freq))
        {
            return;
        }
    ...
    private void RepeatMethod()
    {
        ...
        int relax_interval = int.Parse(NetInfoMgr.instance.NetServerData.relax_interval);
    ...
    public void NoThanksAddCount(int customIndex = 0)
    {
        ...
        int nextlevel_interval = int.Parse(NetInfoMgr.instance.NetServerData.nextlevel_interval);
    ...
    private void OnApplicationPause(bool pause)
    {
        if (!pause)
        {
            ...
            if (!isShowingAd)
            {
                ...
                int inter_b2f_count = int.Parse(NetInfoMgr.instance.NetServerData.inter_b2f_count);
    ...
    public void autoInterTouch()
    {
        if (autoInter)
        {
            ShowInterStitialAd("11");
        }
    }
    ...
    public void ShowInterStitialAd(string interCode)
    {
       ...
       //     SOHOShopManager.instance.AddTaskValue();
        ...
        //插屏触发基础条件
        if (int.Parse(NetInfoMgr.instance.NetServerData.trial_MaxNum) >= SaveDataManager.GetInt(CConfig.sv_Level_Id))
        {
            Debug.Log("inter_ad:方块最大值不满足调价");
            return;
        }
        if (canShowInter)
        {
            Debug.Log("inter_ad:时间间隔满足 编号为:" + ad_inter_index);
            StartCoroutine(showInterstitial());
        }
        else
        {
            Debug.Log("inter_ad:距离上次插屏时间不到");
        }
    }
    ...
    IEnumerator showInterstitial()
    {
        if (canInter)
        {
            ...
            if (!interReady)
            {
                ...
                canInter = false;
                alreadyShow = false;
                StartCoroutine(nameof(reloadInterAction));
                loadInterstital();
            }
            else
            {
                yield return new WaitForSeconds(float.Parse(NetInfoMgr.instance.NetServerData.inter_delay));
                alreadyShow = false;
                PostEventScript.GetInstance().SendEvent("9102", ad_inter_index);
                if (ad_inter_index == "112")
                {
                    SaveDataManager.SetInt("NoThanksCount", 0);
                }
                showInterstital();
    ...
    public void initAutoPlayTime()
    {
        autoInterTime = 0;
        isAutoStart = true;
        autoInter = false;
        StopCoroutine(nameof(autoPlayInter));
        StartCoroutine(nameof(autoPlayInter));
    }
    ...
    public void showInterstital()
    {
        ...
        ADManager.Instance.isLeave = true;
        string placement = ad_inter_index + "_" + inter_networkname;
        MaxSdk.ShowInterstitial(MAX_INTER_ID, placement);
#endif
    }

    public void resumeAutoPlayTime()
    {
        if (!isAutoStart)
        {
            isAutoStart = true;
    ...
    IEnumerator autoPlayInter()
    {
        while (autoInterTime < float.Parse(NetInfoMgr.instance.NetServerData.relax_interval) || !canShowInter || int.Parse(NetInfoMgr.instance.NetServerData.trial_MaxNum) >= SaveDataManager.GetInt(CConfig.sv_Level_Id))
        {
            yield return new WaitForSeconds(1);
            autoInterTime++;
            //Debug.Log(autoInterTime);
        }
        autoInter = true;
        autoInterTouch();
    }
